...
namespace TestPlanCommon.NFModel
{
    public class NFTestConditionReader : TestConditionReaderBase
    {
        ...
        public void ReadSubFiles()
        {
            try
            {
                string LocSetFilePath = Convert.ToString(cNFTestCondition.DicCalInfo[DataFilePathLib.LocSettingPath]);
                string CalFilePath = Convert.ToString(cNFTestCondition.DicCalInfo[DataFilePathLib.CalPathRF]);
                string BoardlossFilePath = GetTestPlanPath() + @"BOARDLOSS\" + Convert.ToString(cNFTestCondition.DicCalInfo[DataFilePathLib.BoardLossPath]);

                cNFTestCondition.CalList = ReadCalProcedure(LocSetFilePath);
                cNFTestCondition.DicCalInfo[DataFilePathLib.BoardLossPath] = BoardlossFilePath;
        ...
        {
            ReadTCF(TCF_Sheet.ConstPASheetName, TCF_Sheet.ConstPAIndexColumnNo, TCF_Sheet.ConstPATestParaColumnNo, ref cNFTestCondition.DicTestPA, ref cNFTestCondition.DicTestLabel);
        }
        private void ReadCalSheet()
        {
            ReadCalSheet(TCF_Sheet.ConstCalSheetName, TCF_Sheet.ConstCalIndexColumnNo, TCF_Sheet.ConstCalParaColumnNo, ref cNFTestCondition.DicCalInfo);
        }
        private void ReadWafeForm()
        {
            ReadWaveformFilePath(TCF_Sheet.ConstKeyWordSheetName, TCF_Sheet.ConstWaveFormColumnNo, ref cNFTestCondition.DicWaveForm, ref cNFTestCondition.DicWaveFormMutate, ref cNFTestCondition.DicWaveFormAlias);
        }
        private void ReadMipiReg()
        {
            ReadMipiReg(TCF_Sheet.ConstMipiRegSheetName, TCF_Sheet.ConstMipiKeyIndexColumnNo, TCF_Sheet.ConstMipiRegColumnNo, ref cNFTestCondition.DicMipiKey);
        }
        private void ReadPwrBlast()
        {
            ReadPwrBlast(TCF_Sheet.ConstPwrBlastSheetName, TCF_Sheet.ConstPwrBlastIndexColumnNo, TCF_Sheet.ConstPwrBlastColumnNo, ref cNFTestCondition.DicPwrBlast);
        }
